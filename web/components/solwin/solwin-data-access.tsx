...
import { getsolwinProgram, getsolwinProgramId } from '@app/anchor';
...
export function useSolwinProgram() {
  const { connection } = useConnection();
  const { cluster } = useCluster();
  const transactionToast = useTransactionToast();
  const provider = useAnchorProvider();
  const programId = useMemo(
    () => getsolwinProgramId(cluster.network as Cluster),
    [cluster]
  );
  const program = getsolwinProgram(provider);
  ...
  const getExchangeRate = useQuery({
    queryKey: ['get-exchange-rate', { cluster }],
    queryFn: () => program.methods
        .getExchangeRate()
        .accounts({
...
export function useCounterProgramAccount({ account }: { account: PublicKey }) {
  const { cluster } = useCluster();
  const transactionToast = useTransactionToast();
  const { program, accounts } = useSolwinProgram();
